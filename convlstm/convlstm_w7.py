...
model.compile(
    ...
    metrics=[
        ...
        tf.keras.metrics.BinaryAccuracy(name='accuracy', threshold=0.4),
    ]
)
model.summary()
history = model.fit(X_train, y_train, epochs=50, batch_size=16, validation_data=(X_val, y_val), class_weight=class_weights_dict,)
...
# fix threshold
y_pred_fixed = (proba_eval >= 0.4).astype(int)
